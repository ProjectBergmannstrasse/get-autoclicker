#!/usr/bin/env python3
"""AutoClicker v029 — ProjectBergmannstrasse — + Reset button"""
...
def do_reset():
    global count
    count = 0
    try: count_lbl.config(text="Clicks: 0")
    except Exception: pass

root = tk.Tk()
root.title("AutoClicker v029")
root.geometry("360x562")
...
L("Pattern:").pack()
pat_var = tk.StringVar(value="constant"); tk.OptionMenu(root, pat_var, "constant", "jitter", "burst").pack()
L("Position:").pack()
pos_var = tk.StringVar(value="follow"); tk.OptionMenu(root, pos_var, "follow", "lock").pack()
fs_var = tk.IntVar(value=1); tk.Checkbutton(root, text="Failsafe (top-left = stop)", variable=fs_var, bg="#f0f0f0", fg="#000000", selectcolor="#f0f0f0").pack()
L("Hotkey:").pack()
hk_var = tk.StringVar(value="f6"); tk.OptionMenu(root, hk_var, "f6", "f7", "f8").pack()
tk.Button(root, text="Start", command=do_start).pack(pady=4)
tk.Button(root, text="Stop", command=do_stop).pack(pady=4)
tk.Button(root, text="Reset", command=do_reset).pack(pady=4)
