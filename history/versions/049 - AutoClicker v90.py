#!/usr/bin/env python3
"""AutoClicker v090 — ProjectBergmannstrasse — HISTORY: loads sessions (Textual)"""
import threading, time
from textual.app import App, ComposeResult
from textual.containers import Container, Horizontal, VerticalScroll
from textual.widgets import Button, Input, Static, ProgressBar, Switch, TabbedContent, TabPane
try:
...
class AutoClicker(App):
    ...
    #fb-comment { background: #161625; width: 40; height: 3; margin: 0 2; }
    """
        ...
        with TabbedContent():
            with TabPane("MAIN", id="t-main"):
                with VerticalScroll():
                    with Container():
                        ...
                        yield Static(DRIPS[0], classes="drip")
                    with Horizontal():
                    ...
                    with Horizontal():
                        yield Static("eff cps 0.0", classes="hint")
                        yield Static("sessions 0", classes="hint")
                        yield Static("00:00:00", classes="hint")
            ...
            with TabPane("INFO", id="t-info"):
                with VerticalScroll():
                    ...
                    yield Static("  LIMIT = auto-stop after N", classes="cmd")
                    yield Static("SETUP - YOUR SYSTEM", classes="title")
                    yield Static("  detected: your OS", classes="cmd")
                    yield Static("  run: python3 autoclicker.py", classes="cmd")
                    yield Static("PERMISSIONS", classes="title")
                    yield Static("  grant accessibility access", classes="cmd")
                    yield Static("COPY ADDRESSES", classes="title")
                    yield Static("  github  bitcoin  ethereum  solana", classes="cmd")
                    yield Static("INSTALL", classes="title")
                    yield Static("  id  a1b2c3d4", classes="cmd")
                    yield Static("  sha256  9f8e7d...", classes="cmd")
            ...
            with TabPane("HISTORY", id="t-hist"):
                with VerticalScroll():
                    ...
                    yield Static("  2  10:05  200 clicks  18.0s", classes="cmd")
                    yield Static("  3  10:12  340 clicks  27.1s", classes="cmd")
