#!/usr/bin/env python3
"""AutoClicker v106 — ProjectBergmannstrasse — INFO: + platform note (Textual)"""
        ...
        with TabbedContent():
            with TabPane("MAIN", id="t-main"):
                with VerticalScroll():
                    ...
                    with Horizontal():
                        yield Button("◎ CAPTURE LOCK POINT", id="capture")
                        yield Static("not set", classes="hint")
                    with Horizontal():
            ...
            with TabPane("EMULATION", id="t-emu"):
                with VerticalScroll():
                    yield Static("MACRO RECORDER", classes="title")
                    with Horizontal():
                        yield Button("● RECORD", id="emu-record")
                        yield Static("idle", classes="hint")
                    with Horizontal():
                        yield Input(placeholder="name this recording", id="emu-name")
                        yield Button("save", id="emu-save")
                    yield Static("PLAYBACK", classes="title")
                    with Horizontal():
                        yield Static("0.5x", classes="seg")
                        yield Static("▸ 1x", classes="seg seg-on")
                        yield Static("2x", classes="seg")
                    with Horizontal():
                        yield Static("▸ ONCE", classes="seg seg-on")
                        yield Static("LOOP", classes="seg")
                    yield Static("SAVED RECORDINGS", classes="title")
                    with Horizontal():
                        yield Static("roblox farm", classes="cmd")
                        yield Static("▶ play", classes="rec-play")
                        yield Static("✕ delete", classes="rec-del")
            ...
            with TabPane("FEEDBACK", id="t-fb"):
                with VerticalScroll():
                    ...
                    with Horizontal():
                        for _s in range(5):
                            yield Static("★", classes="star")
                    yield Static("comment:", classes="cmd")
