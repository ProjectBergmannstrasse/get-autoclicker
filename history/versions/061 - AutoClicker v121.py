...
v5.7 changes vs v5.6:
  - MACRO PLAYBACK now has a manual-move failsafe. When a recording is
    looping, the pointer is being yanked around, so you could not reach
    the STOP button. Now: just grab the mouse and keep moving it. If you
    fight the macro (hold the cursor >60px away from where it is trying
    to put it) for 3 seconds straight, playback stops on its own. A live
    countdown shows while you do it. Works during loop and single play.
...
class AutoClickerApp(App):
    ...
    def emu_play(self, stem: str) -> None:
        ...
        # Manual-move failsafe: track where the macro *intends* the cursor to be.
        # If the real mouse is dragged away from that point for ~3s, stop playback.
        self._play_expected_pos = None
        self._play_override_since = None
        ...
        # Watcher that stops playback if the user manually moves the mouse
        threading.Thread(target=self._play_failsafe_watch, daemon=True).start()
                ...
                for ev in events:
                    ...
                    try:
                        if ev["type"] == "move":
                            ...
                            self._play_expected_pos = (ev["x"], ev["y"])
                        elif ev["type"] == "click":
                            mc.position = (ev["x"], ev["y"])
                            self._play_expected_pos = (ev["x"], ev["y"])
    ...
    def _play_failsafe_watch(self) -> None:
        """Stop macro playback if the user manually moves the real mouse.

        The macro keeps snapping the cursor to known points. We sample the
        real cursor ~20x/sec; whenever it sits far from where the macro last
        put it (i.e. the user is physically dragging it), we start a timer.
        If that fight lasts MOVE_STOP_SECONDS continuously, playback stops so
        the user never has to reach a button while the cursor is being yanked.
        """
        if not PYNPUT_OK or self.mouse_ctrl is None:
            return
        MOVE_STOP_SECONDS = 3.0     # how long you must fight the macro to stop it
        DRIFT_PX = 60               # how far off counts as "you moved it"
        mc = self.mouse_ctrl
        while self._play_active:
            exp = getattr(self, "_play_expected_pos", None)
            if exp is not None:
                try:
                    ax, ay = mc.position
                except Exception:
                    ax, ay = exp
                dist = ((ax - exp[0]) ** 2 + (ay - exp[1]) ** 2) ** 0.5
                now = time.monotonic()
                if dist > DRIFT_PX:
                    if self._play_override_since is None:
                        self._play_override_since = now
                    held = now - self._play_override_since
                    remain = max(0.0, MOVE_STOP_SECONDS - held)
                    try:
                        self.call_from_thread(
                            self._update_emu_status,
                            f"[#ff9100]keep moving your mouse to stop… "
                            f"{remain:0.1f}s[/]")
                    except Exception:
                        pass
                    if held >= MOVE_STOP_SECONDS:
                        self._play_active = False
                        try:
                            self.call_from_thread(
                                self._update_emu_status,
                                "[#ff1744]■ playback stopped (you took over the mouse)[/]")
                        except Exception:
                            pass
                        break
                else:
